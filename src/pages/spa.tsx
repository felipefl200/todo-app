...
function Spa() {
    ...
    useEffect(() => {
        let loadData = async () => {
            await axios('/api/todo').then(res => setTodos(res.data))
        }
        loadData()
    }, [])
                    ...
                    {todos?.map((item, index) => (
                        <div key={item.id} className="flex justify-center">
                            <div className=" relative justify-center mt-6">
                                ...
                                <span className="absolute -left-3 -top-3 bg-green-500 flex justify-center items-center rounded-full w-8 h-8 text-gray-50 font-bold">{item.id}</span>
